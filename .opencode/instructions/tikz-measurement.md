<!-- Original: tikz-measurement.md — path-scoped (Slides/**/*.tex, Figures/**/*.tex, Preambles/**/*.tex, scripts/**/*.py, scripts/**/*.R) — TikZ measurement and collision formulas -->
...
- **`/extract-tikz` and `/new-diagram`** — both run a Step 1 prevention pre-check against the rules in [`.opencode/instructions/tikz-prevention.md`](tikz-prevention.md) (P3 bare `scale=`, P4 missing directional keyword) before compiling. Both skills use identical grep patterns so behavior doesn't drift.
...
- **`quality_score.py`** — see [`.opencode/instructions/quality-gates.md`](quality-gates.md) for the authoritative TikZ rubric. A label/arrow overlap finding currently costs −5 in the Quarto and Beamer rubrics.
