<!-- Original: orchestrator-protocol.md — always-on rule describing the 6-step contractor loop pattern -->
...
- **Do NOT treat this phrase alone as commit authorization.** Commits still require an explicit `/commit` invocation or an unambiguous user request to commit — see [`.opencode/skills/commit/SKILL.md`](../skills/commit/SKILL.md) for the allowed trigger phrases.

## Cross-references

- `.opencode/instructions/plan-first-workflow.md` — when to enter plan mode before invoking a skill.
- `.opencode/instructions/quality-gates.md` — threshold definitions.
- `.opencode/instructions/cross-artifact-review.md` — paper ↔ code dependency-graph pattern.
