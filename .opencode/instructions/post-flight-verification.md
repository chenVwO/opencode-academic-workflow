<!-- Original: post-flight-verification.md — anti-hallucination verification protocol -->

# Post-Flight Verification (anti-hallucination)

Symmetric partner to **Pre-Flight Reports** (`.opencode/instructions/content-invariants.md` + skill-level `## Phase 0`). Where Pre-Flight proves inputs were read *before* work, Post-Flight proves the output's factual claims hold *after* drafting — before the skill returns to the user.
...
- `.opencode/agents/claim-verifier.md` — the forked verifier.
- `.opencode/skills/verify-claims/SKILL.md` — user-facing wrapper for ad-hoc verification of any text.
- `.opencode/instructions/content-invariants.md` — Pre-Flight (input side).
- `.opencode/instructions/cross-artifact-review.md` — pattern-based; Post-Flight is draft-based.
- `.opencode/instructions/summary-parity.md` — rule against enumerative summaries drifting from their bodies; Post-Flight is the factual equivalent for draft content.
