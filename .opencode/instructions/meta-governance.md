<!-- Original: meta-governance.md — always-on rule for repository dual nature (working project + template) -->
...
### .opencode/personal-memory.md (gitignored, local only)
...
- Builds `.opencode/personal-memory.md` specific to desktop setup

**Machine B (laptop):**
- Clone same repo → gets same MEMORY.md ✓
- Gets same infrastructure ✓
- Builds DIFFERENT `.opencode/personal-memory.md` for laptop setup
...
| Machine-specific setup | ❌ No | .opencode/personal-memory.md | ❌ No (gitignored) |
| Templates (generic) | ✅ Yes | templates/ | ✅ Yes |
| Skills (generic) | ✅ Yes | .opencode/skills/ | ✅ Yes |
| Rules (path-scoped, generic) | ✅ Yes | .opencode/instructions/ | ✅ Yes |
| Agents (generic) | ✅ Yes | .opencode/agents/ | ✅ Yes |
| Hooks (generic behavior) | ✅ Yes | .opencode/plugins/ | ✅ Yes |
| Session logs | ✅ Yes | quality_reports/session_logs/ | ✅ Yes |
| Plans | ✅ Yes | quality_reports/plans/ | ✅ Yes |
| Local settings | ❌ No | opencode.local.json | ❌ No (gitignored) |
| Session state | ❌ No | .opencode/state/ | ❌ No (gitignored) |
...
- Keep specific learnings local (.opencode/personal-memory.md, gitignored)
