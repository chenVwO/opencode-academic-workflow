...
```powershell
$VSCODE_WS = "$env:PROJECT_DIR\.vscode\settings.json"
$CLI_PROJECT = "$env:PROJECT_DIR\.opencode\settings.json"
$CLI_LOCAL = "$env:PROJECT_DIR\.opencode\settings.local.json"
```
...
```powershell
# VSCode user (platform-dependent path)
$isWindows = [Environment]::OSVersion.Platform -eq [PlatformID]::Win32NT
$isMac = [Environment]::OSVersion.Platform -eq [PlatformID]::MacOSX
$isLinux = [Environment]::OSVersion.Platform -eq [PlatformID]::Unix

if ($isMac) {
  $VSCODE_USER = "$HOME/Library/Application Support/Code/User/settings.json"
} elseif ($isLinux) {
  $VSCODE_USER = "$HOME/.config/Code/User/settings.json"
} elseif ($isWindows) {
  $VSCODE_USER = "$env:APPDATA\Code\User\settings.json"
} else {
  $VSCODE_USER = ""
}

$CLI_USER = "$HOME\.opencode\settings.json"
```
...
The live in-session mode is exposed via the status line (see `.opencode/plugins/statusline.sh`). Tell the user:
...
If the status line isn't configured, emit a warning and point at `.opencode/plugins/statusline.sh`.
...
- If `$env:PROJECT_DIR` is unset, fall back to `git rev-parse --show-toplevel`.
