...
- For the narrative *what happened* — that lives in `quality_reports/session_logs/` (see `.opencode/instructions/session-logging.md`).
...
- `.opencode/skills/checkpoint/SKILL.md:42` — body draft, needs trigger-phrase tightening
...
If the user says "yes" / "all" / "1 and 3" — append to MEMORY.md (root, the committed one) using the `[LEARN]` format. If the candidate is machine-specific (paths, tool versions, personal preference), recommend the user route it to `.opencode/state/personal-memory.md` instead per `.opencode/instructions/meta-governance.md`.
...
- `.opencode/instructions/session-logging.md` — narrative companion. **Do not duplicate** — the checkpoint references the latest session log by path; it does not re-tell the session story.
- `.opencode/instructions/plan-first-workflow.md` — checkpoint reads the active plan; if no plan exists, recommend the user enter plan mode before invoking `/checkpoint`.
- `templates/decision-record.md` — for *why we chose A over B*, not for *where we are*.
- `.opencode/plugins/pre-compact.py` — when `CLAUDE_PRECOMPACT_BLOCK_ON_DRAFT=1` is set, the PreCompact hook will block compaction once per DRAFT plan. `/checkpoint` is the right thing to run when that block fires.
...
**Result:** Next session: `opencode --continue`, then `read quality_reports/checkpoints/2026-04-27_v180-polisci.md and start at action 1`.
