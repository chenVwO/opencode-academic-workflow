...
# Has TikZ diagrams?
$has_tikz = (Select-String -LiteralPath $FILE -Pattern '\\begin{tikzpicture}' -SimpleMatch).Matches.Count

# For .qmd: is there a paired .tex in Slides/?
$has_tex_pair = $false
if ($FILE -like "*.qmd") {
  $base = [System.IO.Path]::GetFileNameWithoutExtension($FILE)
  $candidates = @("Slides/$base.tex", "Slides/Lecture$base.tex")
...
if ($FILE -like "*.tex") {
  $base = [System.IO.Path]::GetFileNameWithoutExtension($FILE)
  $candidates = @("Quarto/$base.qmd", "Quarto/$($base -replace '^Lecture', '').qmd")
  foreach ($candidate in $candidates) {
    if (Test-Path -LiteralPath $candidate) {
      $has_qmd_pair = $true
      $qmd_pair = $candidate
...
$has_r = $false
if (Select-String -LiteralPath $FILE -Pattern "```\{r|source\(.*\.R\)" -Quiet) {
  $has_r = $true
}
```
...
Before spawning the substance-review agent on a `.tex` file, verify `.opencode/agents/domain-reviewer.md` has been customized for this project. The ship-state domain-reviewer is a **template** — running it unmodified produces generic "are assumptions stated?" feedback, not real domain review.
...
- The five lenses are identical to the shipped template's wording (diff against `.opencode/agents/domain-reviewer.md` on the v1.3.0 tag — if zero lines changed, it's still template).
...
stated?") rather than field-specific review. Options:
  ...
  1. Customize .opencode/agents/domain-reviewer.md — replace the 5 lenses
