...
Focus: **all** executable code in the repo — `.opencode/plugins/*.py`, `.opencode/plugins/*.sh`, `scripts/*.py`, `scripts/*.sh`, `.opencode/plugins/*.sh`. Not just `.opencode/plugins/` — when PR #93 added new code under `scripts/`, the original narrow scope meant Copilot + Codex caught 5 bugs the audit missed.

Hook-specific checks (Stop/PreToolUse/SessionStart protocols, `CLAUDE_PROJECT_DIR` usage, hash-length consistency) apply only to `.opencode/plugins/`. Everything below applies to ALL executable code:

- No remaining `/tmp/` usage in anything that manages state (should use `~/.config/opencode/sessions/`)
...
Focus: `.opencode/skills/*/SKILL.md` and `.opencode/instructions/*.md`
...
Copy-Item guide/workflow-guide.html docs/workflow-guide.html
```
...
| State in /tmp/ | All Python hooks | Should use `~/.config/opencode/sessions/<hash>/` |
