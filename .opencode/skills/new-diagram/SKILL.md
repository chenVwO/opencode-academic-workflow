...
$DST = if ($args[1]) { $args[1] } else { "Figures/new_diagram.tex" }
...
```bash
python scripts/check-tikz-prevention.py "$DST"
```
...
```powershell
Set-Location -Path (Split-Path $DST -Parent)
& xelatex -interaction=nonstopmode (Split-Path $DST -Leaf) 2>&1 | Out-File -FilePath "$env:TEMP\tikz-compile.log"
```

Check exit code and `*.pdf` file size. If compile fails, read `$env:TEMP\tikz-compile.log` and fix the `.tex` source.
...
```powershell
$pdfPath = [System.IO.Path]::ChangeExtension($DST, ".pdf")
$svgPath = [System.IO.Path]::ChangeExtension($DST, ".svg")
& pdf2svg $pdfPath $svgPath 1
```
...
```powershell
Set-Location -Path (Split-Path $DST -Parent)
Remove-Item -Force *.aux, *.log, *.out, *.synctex.gz
```
...
- [`.opencode/instructions/tikz-prevention.md`](../../rules/tikz-prevention.md) — the P1–P6 authoring rules.
- [`.opencode/instructions/tikz-measurement.md`](../../rules/tikz-measurement.md) — the six-pass protocol with formulas the reviewer cites.
- [`.opencode/instructions/tikz-visual-quality.md`](../../rules/tikz-visual-quality.md) — general visual standards.
- [`.opencode/skills/extract-tikz/SKILL.md`](../extract-tikz/SKILL.md) — for pulling TikZ out of an existing Beamer deck instead of creating new.
