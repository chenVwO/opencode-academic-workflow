...
```bash
python scripts/quality_score.py <changed-file-paths>
```
...
**Runs unconditionally.** Enforces that count claims (`"14 agents, 28 skills, 24 rules, 6 hooks"` and siblings) across README.md, CLAUDE.md, the guide source + rendered HTML, the landing page, and the skill template all agree with the on-disk counts of `.opencode/{skills,agents,instructions,plugins}`:

```bash
./scripts/check-surface-sync.ps1
```
...
Do NOT stage `.opencode/settings.local.json` or any files containing secrets.
