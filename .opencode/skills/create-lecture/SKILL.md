...
Inputs to read:
- `.opencode/instructions/knowledge-base-template.md` — notation registry, narrative arc, applications
- `.opencode/instructions/content-invariants.md` — INV-1..INV-8 govern slide content
...
**First-lecture fallback (fresh fork, empty knowledge base).** If `.opencode/instructions/knowledge-base-template.md` still has unfilled placeholder tables (no notation registry entries, no applications, no prior lectures in `Slides/`), do NOT halt waiting for it. Instead:
