...
Before compiling, verify every `\begin{tikzpicture}` block in `Figures/$ARGUMENTS/extract_tikz.tex` satisfies the prevention rules in [`.opencode/instructions/tikz-prevention.md`](../../rules/tikz-prevention.md). The pre-check is a small Python script shared with `/new-diagram` so both skills enforce identical behavior:

```bash
python scripts/check-tikz-prevention.py "Figures/$ARGUMENTS/extract_tikz.tex"
```
...
```bash
Set-Location -Path Figures/$ARGUMENTS
```

### Step 3: Compile the extract_tikz.tex file
```bash
$env:TEXINPUTS="../../Preambles;$env:TEXINPUTS"; & xelatex -interaction=nonstopmode extract_tikz.tex
```
...
```powershell
$PAGES = $(pdfinfo extract_tikz.pdf | Select-String "Pages:" | ForEach-Object { $_.ToString().Split(":")[1].Trim() })
...
```bash
Set-Location -Path ..\..
& ".\scripts\sync-to-docs.ps1" $ARGUMENTS
```
...
Spawn the **tikz-reviewer** agent (via `Task` with `subagent_type=tikz-reviewer`) on the TikZ source blocks to catch label overlaps, geometric errors, and visual inconsistencies. The reviewer cites specific passes and formulas from [`.opencode/instructions/tikz-measurement.md`](../../rules/tikz-measurement.md). If it returns **NEEDS REVISION** or **REJECTED**, loop:
...
TikZ diagrams MUST be edited in the Beamer `.tex` file first, then copied verbatim to `extract_tikz.tex`. See `.opencode/instructions/single-source-of-truth.md`.
